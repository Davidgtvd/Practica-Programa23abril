...
export default function ArtistaBandaListView() {
  ...
  useEffect(() => {
    ArtistaBandaServices.listAll().then(function (data) {
      //items.values = data;
      setItems(data);
    });
  }, []);
  ...
  const order = (event, columnId) => {
    ...
    ArtistaBandaServices.order(columnId, dir).then(function (data) {
      setItems(data);
    });
  }
  ...
  const search = async () => {
    try {
      ...
      ArtistaBandaServices.search(criterio.value, texto.value, 0).then(function (data) {
        setItems(data);
      });
